...
**`board/` on disk — confirmed for the deterministic paths, unverified for the material path.** `agent/lib/board.ts`'s `readBoard`/`writeBoardAtomic`, and the custom `GET /board` / `GET /board/md` route handlers in `agent/channels/board.ts`, all use plain host-process `node:fs` (`node:fs/promises`) directly — there is no sandbox indirection in that code path. This was live-verified end to end for the deterministic (all-clear and degraded-fallback) tick paths via `eve dev`: a real tick ran, `writeBoardAtomic` wrote `board/board.json` on the host filesystem, and `curl /board` on the host read back the same file. For those paths, the earlier claim that board files live in "the sandbox filesystem (`/workspace`), reached only through sandbox tools" was simply wrong — no sandbox is involved.

**UNVERIFIED: the material path**, where the Haiku pr-board agent calls the `board` tool (`agent/tools/board.ts`) itself from inside its own eve-managed execution context, has never been confirmed to write to the *same* host filesystem the tick/HTTP-route code reads from. If eve executes a `defineTool`'s `execute()` inside a sandboxed filesystem rather than the host process — plausible given eve's sandbox backends (fact 4 below) — the agent's board write could land somewhere `readBoard()` never sees. This would not crash anything: `runTick`'s material-path retry would simply find no fresh board, exhaust its retry, and fall through to the deterministic degraded-fallback board (graceful, per spec §2.4). But it would mean the LLM summarization step silently never takes effect in production, without producing any error. Resolving this requires a real deploy-time smoke test — trigger a material tick via the `board` tool, then `curl /board` on the host and confirm the LLM-authored summary is actually present — which needs deployment infrastructure that doesn't exist yet. Tracked as a follow-up (see bead filed against this repo); the deployment tasks should not assume this is safe until that smoke test passes.
