...
exists for private data sources that shouldn't be in a public repo at all,
not even as a stub. The external package never imports this repo and this
...
`@agentic-research/vespers-core`'s `SnapshotSource` earned "generic": a second real
...
### `@agentic-research/vespers-core`: the portable tick/fold engine
...
npm consumers, deliberately avoiding GitHub Packages auth for downstream
installs.
...
`graphql/sources/{github,linear}.graphql` +
